...
//
// Neither path had them. `gish -c 'echo $1' _ hi` printed nothing, and
// `gish script.sh a b` left $1 empty and $# at zero — so any script
...
func TestDashCPassesPositionalParameters(t *testing.T) {
	...
	for _, tc := range paramCases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			gishOut, gishCode := runShellArgs(t, gish, tc.script, tc.args)
			bashOut, bashCode := runShellArgs(t, bash, tc.script, tc.args)
			if gishOut != bashOut || gishCode != bashCode {
				t.Errorf("differs from bash\n  bash: %q (exit %d)\n  gish: %q (exit %d)",
					bashOut, bashCode, gishOut, gishCode)
			}
		})
	}
}
...
func TestDashCWithParametersStaysQuiet(t *testing.T) {
	...
	if !r.Pass {
		t.Errorf("%s\n  bash: %q\n  gish: %q", r.Reason, r.BashOut, r.GishOut)
	}
}
