...
func TestPrintfInvalidNumbersMatchBash(t *testing.T) {
	...
	// as a *warning* with status 0. gish follows modern bash, which is
	...
	for _, script := range printfNumberCases {
		t.Run(script, func(t *testing.T) {
			t.Parallel()
			gishOut, gishCode := runNormalized(t, gish, script)
			bashOut, bashCode := runNormalized(t, bash, script)
			if gishOut != bashOut || gishCode != bashCode {
				t.Errorf("gish %q (exit %d)\nbash %q (exit %d)",
					gishOut, gishCode, bashOut, bashCode)
			}
		})
	}
}
...
// own diagnostics. It is stripped before comparing because gish
