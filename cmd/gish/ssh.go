...
// `gish ssh [user@]host` (#98): the answer to the most-cited blocker to
...
const sshUsage = `usage: gish ssh [ssh-flags] [user@]host
       gish ssh --uninstall [user@]host

Bring gish to a host you only have ssh to: probe it, copy one static
binary plus your prompt settings into a cache dir under your home
there, and open an interactive gish. Repeat visits copy nothing.

  --uninstall   remove everything gish left on the host, then exit
...
Whether to bring gish is GISH_SSH_BRING: ask (default, remembered per
...
func runSSH(ctx context.Context, args []string) int {
	...
	if host == "" {
		fmt.Fprintln(os.Stderr, "gish ssh: no host given\n"+sshUsage)
		return 2
	}
	...
	if uninstall {
		...
		if err != nil {
			fmt.Fprintln(os.Stderr, "gish ssh:", err)
			return 1
		}
		if len(removed) == 0 {
			fmt.Fprintf(os.Stderr, "gish ssh: nothing of gish's on %s\n", host)
		}
		for _, d := range removed {
			fmt.Fprintf(os.Stderr, "gish ssh: removed %s:%s\n", host, d)
		}
...
// parseSSHArgs splits gish's flags from ssh's. gish claims only long
...
func parseSSHArgs(args []string) sshInvocation {
	...
	for i := 0; i < len(args); i++ {
		...
		case strings.HasPrefix(a, "-"):
			...
			// argument, or `gish ssh -o ConnectTimeout=1 host` would read
...
// exitCodeOf turns a child's result into our exit code, so `gish ssh`
