...
func TestThemedPromptAndGhostTextRenderStyled(t *testing.T) {
	...
	// The gish theme with a right prompt: both halves are set by the
	// theme and rendered by the editor, which is the join no unit test
	// on either side can reach.
	if err := os.WriteFile(rc, []byte("GISH_THEME=gish\nGISH_THEME_RPROMPT=time\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if s.seen(ghost) {
		t.Errorf("GISH_SUGGEST=off still rendered ghost text; output:\n%s", out)
	}
}
