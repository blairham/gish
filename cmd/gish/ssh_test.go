...
// `gish ssh` has to be a transparent front for ssh: anything gish does
// not claim is passed through untouched. The failure that matters is
// getting the *host* wrong — `gish ssh -o ConnectTimeout=1 web` reading
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantFlag != nil && !tt.wantFlag(got) {
				t.Errorf("gish flag not set from %q", tt.args)
			}
		})
	}
}

// A gish flag must never be forwarded to ssh, which would reject it.
func TestGishFlagsAreNotForwardedToSSH(t *testing.T) {
