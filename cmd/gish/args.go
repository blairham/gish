...
//
// gish parsed its own argv with the `flag` package, which models Go
...
//     `echo hi`. `flag` made "-l" the value of -c, so gish ran a command
...
//
// So argv is parsed here by hand. The gish-specific long options keep
...
// gish's own; the short set below is the POSIX one other programs use.
...
const usage = `usage: gish [options] [script [args…]]
  ...
  --sandbox profile run every external command under a sandbox profile
                    (none opts out of the gish-agent default)
  --rc file         read startup settings from file
  --restore id      start in the directory of a recorded session
  --remote-session  this session was brought here by gish ssh
  --version         print version and exit

Short options cluster: gish -lc 'echo hi' is a login shell running one
