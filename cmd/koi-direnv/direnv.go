...
// exactly right here, because reverting is koi's job (#12 snapshots
...
// trust flow. koi's own `trust allow` calls this, so a user approves
// once and both systems record it.
//
// Both key on content, which is what makes one gesture safe: editing the
// .envrc re-blocks direnv *and* changes koi's diff hash, so the two
