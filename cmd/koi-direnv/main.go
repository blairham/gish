// Command koi-direnv drives the user's own direnv through the
...
//   - Owned by koi: the *cd moment* (no `eval "$(direnv hook bash)"` —
...
//
// The diff arrives with koi's guarantees intact: nothing applies until
...
// which tells the host there is something here awaiting approval. koi's
...
func (p provider) EnvDiff(ctx context.Context, req *pluginapi.EnvDiffRequest) (*pluginapi.EnvDiffResponse, error) {
	...
	case stateNoRC, stateDenied:
		...
		// would be nagging, and koi is not the authority here.
		return &pluginapi.EnvDiffResponse{}, nil
	case stateBlocked:
...
// approved in koi, so direnv is told too.
//
...
// koi thinks a directory is trusted and direnv silently exports
...
func (p provider) Allow(ctx context.Context, req *pluginapi.AllowRequest) (*pluginapi.AllowResponse, error) {
	...
	if err := p.bridge.allow(ctx, dir, req.GetEnv()); err != nil {
		// The host still applies the diff it showed the user — koi's
		// own record is authoritative for koi — but it needs to say
...
func newPlugin() pluginsdk.Plugin {
	p := pluginsdk.Plugin{Env: provider{bridge: &bridge{}}}
