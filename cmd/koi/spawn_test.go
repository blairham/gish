...
func TestSpawnFormsOtherProgramsUse(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if code != 0 {
				t.Fatalf("koi %q: exit %d\nstderr: %s", tc.args, code, stderr)
			}
			if stdout != tc.want {
				t.Errorf("koi %q: stdout=%q, want %q", tc.args, stdout, tc.want)
			}
		})
	}
}
...
// the substrate built with MustCompile — that reached koi through a
...
func TestInterpreterPanicDoesNotKillTheShell(t *testing.T) {
	...
	const panics = `__koi_panic_probe`
	...
	t.Run("in a login profile", func(t *testing.T) {
		...
		stdout, stderr, code := runC(t, bin, append(env, "KOI_PROFILE="+profile), "-lc", "echo alive")
		if stdout != "alive\n" || code != 0 {
	...
	// KOI_DEBUG opts into the stack, for a bug report.
	t.Run("stack behind KOI_DEBUG", func(t *testing.T) {
		t.Parallel()
		_, stderr, _ := runC(t, bin, append(env, "KOI_DEBUG=1"), "-c", panics)
		if !strings.Contains(stderr, "goroutine ") {
			t.Errorf("KOI_DEBUG=1 did not include the stack:\n%s", stderr)
		}
	})
}
