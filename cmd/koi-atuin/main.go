// Command koi-atuin bridges the user's own atuin installation into
// koi as a tier-2 HistoryBackend (#97).
//
...
// and koi reimplementing sync would inherit none of it. The user's own
...
// failure: Append declines and Search returns nothing, so a koi with
// this plugin installed but atuin uninstalled behaves exactly like a
// koi without the plugin.
...
//
// koi hands over a *finished* command; atuin models a command as start
...
// mirrored entry's timestamp is when koi told atuin about it, not when
...
func (b backend) Search(req *pluginapi.SearchRequest, stream pluginapi.HistoryBackend_SearchServer) error {
	...
	if err != nil {
		...
		// atuin is missing or misconfigured gets koi's own history, not
		// a broken ctrl-r.
