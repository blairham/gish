// Command gish-acp is the AIProvider plugin for any ACP agent (#167,
// role 1).
//
// gish-claude drives one vendor's CLI. This drives a *protocol*: any
...
// gish-specific code on either side.
//
// It is a plugin, and it is deletable if ACP does not take. The
// *outbound* role (an agent executing inside gish) is core instead,
...
//
// The agent to drive is $GISH_ACP_AGENT (a command line), defaulting to
// `claude-code-acp`.
package main
...
func agentArgv() []string {
	if cmd := strings.Fields(os.Getenv("GISH_ACP_AGENT")); len(cmd) > 0 {
		return cmd
	}
...
// following comment line as the rationale — the same shape gish-claude
