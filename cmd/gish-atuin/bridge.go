...
// keeps working — the same posture gish-carapace takes.
...
// searchFormat asks for exactly the fields gish's HistoryEntry carries.
...
//
// **atuin's --duration is nanoseconds**, and gish's HistoryEntry carries
...
func endArgs(id string, exitCode int32, durationMs int64) []string {
	...
	if durationMs > 0 {
		...
		// gish not having measured a duration is different from the
...
func (b *bridge) search(ctx context.Context, query, cwd string, limit int, prefixOnly bool) ([]searchResult, error) {
	...
	if cwd != "" {
		...
		// and gish's picker shows the directory column regardless.
