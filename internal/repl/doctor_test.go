...
func runDoctorScript(t *testing.T, src string) (stdout string, err error) {
	t.Helper()
	base := t.TempDir()
	t.Setenv("KOI_RC", filepath.Join(base, "koirc"))
...
func TestDoctorWarnsOnBadThemeValues(t *testing.T) {
	out, err := runDoctorScript(t,
		"KOI_THEME=p10k KOI_THEME_COLOR_DIR='; rm' KOI_THEME_LINES=9\ndoctor\n")
	if err != nil {
		t.Fatalf("warnings must not fail doctor: %v\n%s", err, out)
	}
	for _, want := range []string{"⚠", "DIR", "KOI_THEME_LINES"} {
		if !strings.Contains(out, want) {
...
func TestDoctorWarnsOnMissingStarship(t *testing.T) {
	...
	out, err := runDoctorScript(t, "KOI_THEME=starship\ndoctor\n")
	if err != nil {
...
func TestDoctorFlagsNonExecutablePlugin(t *testing.T) {
	base := t.TempDir()
	plugins := filepath.Join(base, "data", "koi", "plugins")
	if err := os.MkdirAll(plugins, 0o755); err != nil {
		t.Fatal(err)
	}
	// Not a plugin candidate on any platform: no exec bit on unix, no
	// executable extension on Windows.
	if err := os.WriteFile(filepath.Join(plugins, "koi-thing"), []byte("#!/bin/sh\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	t.Setenv("KOI_RC", filepath.Join(base, "koirc"))
	t.Setenv("XDG_DATA_HOME", filepath.Join(base, "data"))
...
func TestDoctorWarnsOnUnparsableHistoryTail(t *testing.T) {
	base := t.TempDir()
	histDir := filepath.Join(base, "data", "koi")
