...
type HandlerContext struct {
	...
	// ExtraFiles holds the files open on the descriptors above 2, where entry
	// i is descriptor 3+i, matching [os/exec.Cmd.ExtraFiles]. A nil entry is a
	// descriptor which is not open, so gaps are preserved rather than filled.
	//
	// Note that a descriptor opened by an [OpenHandlerFunc] which does not
	// return an [*os.File] cannot be given to another process, so it is nil
	// here while still usable by the shell itself.
	ExtraFiles []*os.File
